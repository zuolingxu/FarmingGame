...
Gate::Gate(MapLayer* parent, const Vec<int>& pos, std::string NM, std::string NP, std::string FC)
    : parent(parent)
    , MapObject(pos)
    , NextMap(NM)
    , NextPosition(NP)
...
{
    if (val.HasMember("NextMap") && val["NextMap"].IsString() && val.HasMember("NextPosition") && val["NextPosition"].IsString()) {
...
void Gate::interact()
{
    ...
    if (facade) {
        ...
        if (Function == "for_sleep") {
            facade->changeScene(NextMap, NextPosition);
            TimeManager::getInstance()->sleep(); // TimeManager logic is not in facade
        ...
        CCLOG("Error: GameFacade instance is null!");
    }
}
