...
Crop::Crop(MapLayer* parent) : parent_(parent), LiveDay(1), MaturationDay(7)
{
...
Crop* Crop::createByPlayer(const cocos2d::Vec2& position, MapLayer* parent, Land* land) {
    Crop* crop = new Crop(parent);
    crop->saveToArchive(position, land); // 传入 Land 对象
    return crop;
}
...
void Crop::saveToArchive(const cocos2d::Vec2& position, Land* land) {
    ...
    cropInfo.AddMember("Water", land->isWatered(), archiveDoc->GetAllocator()); // 使用 Land 的 Water 状态
    cropInfo.AddMember("Fertilizer", land->isFertilized(), archiveDoc->GetAllocator()); // 使用 Land 的 Fertilizer 状态
